...
domain-specific \gls{lora}~\cite{hu2022lora} fine-tuning, rank 2048, learning rate $1e^{-4}$, context size $4096$, 3 epochs.

Deepseek-Coder-7B-Instruct v1.5~\cite{guo2024deepseekcoder}

The first $10$ benchmark results originate from the work VerilogEval V2~\cite{10.1145/3718088}

\begin{table}[!htb]
{
    \centering
    
    \caption{Performance Benchmarks of \glspl{llm} in Verilog Generation with VerilogEval V2~\cite{10.1145/3718088}.}
    \label{tab:ve2_benchmark}
...
\MyLLM-DeepSeek-Coder CL-1 & 7B & \bfseries 67.3\% & \bfseries 62.6\% & \bfseries 70.5\% & \bfseries 62.3\% \\\toprule\bottomrule

\MyLLM-DeepSeek-Coder CL-2 & 7B &  51.9\% &  55.3\%&  55.1\%&  55.2\%\\
\midrule
\MyLLM-DeepSeek-Coder CL-3 & 7B &  53.2\% &  53.1\%&  59.6\%&  52.4\%\\
\midrule
\MyLLM-DeepSeek-Coder CL-4& 7B &  49.4\% &  49.5\%&  53.2\%&  50.8\%\\
\midrule
\MyLLM-DeepSeek-Coder CL-5& 7B &  46.8\% &  49.7\%&  50\%&  47\%\\
\bottomrule
\end{tabular}
}
...
\ref{fig:ve_stat}, an \gls{slm} should not be considered to train with complex-logic \gls{rtl} modules.
